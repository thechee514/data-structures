...
public class SequenceTests {
	...
	private static void testBasic() {
		...
		s1.addAfter("A");
		Testing.assertEquals(
				"There should be a current element after adding an element",
				true, s1.isCurrent());
		s1.advance();
		Testing.assertEquals(
				"There shouldn't be a current element after advancing", false,
				s1.isCurrent());
	}
	...
	private static void testAdd() {
		...
		s1.start();
		Testing.assertEquals(
				"addAfter should create element at end of sequence", "^_^",
				s1.getCurrent());
	...
	private static void testConcatenation() {
		...
		s3.addAfter("o.o");
		Testing.assertEquals(
				"Modifying concatenation doesn't change first sequence",
				"{O.O, >O.O} (capacity = 2)", s1.toString());
		Testing.assertEquals(
				"Modifying concatenation doesn't change second sequence",
				"{^.^, >^.^} (capacity = 2)", s2.toString());
		Testing.assertEquals(
				"Modifying concatenation changes concactenated sequence",
				"{O.O, O.O, ^.^, ^.^, o.o, >o.o} (capacity = 9)", s3.toString());
	}
